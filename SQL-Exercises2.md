...
```
select count（distinct date）/7 from table;
select count(order) from table;
select count/count (distinct date) as ave_order from table;
...
![1567474955136.png](https://github.com/Jaconhunt/SQL-DataAnalysis/blob/master/SQL/1567474955136.png?raw=true)
...
![1567477233715.png](https://github.com/Jaconhunt/SQL-DataAnalysis/blob/master/SQL/1567477233715.png?raw=true)

7.查询数据表（order），基本字段如下：省，市，店铺名称，订单数。要按”市”排序，需要看到每一个市的订单排名前三名的门店及订单数。

```
select city, name, order_num
from order
group by city
order by order_num desc limit 3;
```

8.用户登陆日志表tabel1，包含user_id，log_time，session_id，plat (**练习时间函数，难题**)
...
```
#1.分析，每天平均登陆用户，即统计1-30日登陆用户数，累加再平均
...
9.交易表结构user_id（用户ID），order_id（订单），pay_time（付款时间），order_amount（金额）。**练习时间函数，难题**
...
```
select a.use_id,a.goods_id as agoods_id,b.goods_id as bgoods_id
from a join b 
on a.user_id = b.user_id and a.goods_id = b.goods_id;
#此写法并未验证
```

11.对于通过不同渠道拉新进来的用户，经过一段时间许多用户可能流水，而留下来的用户称之为留存用户。分析用户留存是拉新和用户运营的重要指标。假如我们有一张用户访问表：person_visit，记录了所有用户的访问信息，包含字段：用户id：user_id，访问时间：visit_date，访问页面：page_name，访问渠道（android,ios）：plat等。**练习时间函数，难题**
...
```
select group1.class
from (select group.class,count(group.id) as group_num
from group
group by group.class) as group1
where group1. group_num<1000;
#此写法并未验证
```
...
![1567433734541.png](https://github.com/Jaconhunt/SQL-DataAnalysis/blob/master/SQL/1567433734541.png?raw=true)


14. 实现行列转换

    ![1567409054541.png](https://github.com/Jaconhunt/SQL-DataAnalysis/blob/master/SQL/1567409054541.png?raw=true)

![1567410562400.png](https://github.com/Jaconhunt/SQL-DataAnalysis/blob/master/SQL/1567410562400.png?raw=true)
...
[7] sql连续几天，https://zhuanlan.zhihu.com/p/49285570